...
object RecipeRegistry {
    ...
    // generate-fingerprints.sh can grep the pin set. The oldest entry is the bridge
    // compileOnly floor (gradle/libs.versions.toml okhttpMin). CI matrix in
    // .github/workflows/pr.yml must list the same versions. Older than the oldest
    // entry is unsupported (okhttp 4 and 5.0–5.3); newer than the oldest and not in
    // the map is UNTESTED.
