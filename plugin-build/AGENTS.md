...
Read this first: this composite build (own `settings.gradle.kts` and `gradlew`) contains the
Gradle plugin that rewrites exactly one OkHttp method at build time and fails closed on
anything unexpected. Product code lives in `plugin/src/main/kotlin/dev/okhttpcronet/plugin/`.
...
`plugin-build/scripts/generate-fingerprints.sh <version>` (downloads the artifacts, writes
goldens under `plugin/src/test/resources/stock/<version>/`, regenerates
...
Fixtures live in `plugin/src/test/fixtures/sample-app` (applies `dev.okhttpcronet.transport`,
...
classpath; that wiring already exists in `plugin/build.gradle.kts`. TestKit forks must use
