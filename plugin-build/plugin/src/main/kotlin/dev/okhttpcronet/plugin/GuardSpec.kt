...
fun GuardSpec.verify(insns: List<String>): List<String> {
    ...
    for (rule in requiredInvokes) {
        if (insns.count { it.startsWith(rule.insnPrefix) } != rule.count) {
            val name = rule.insnPrefix.substringAfter(' ').substringBefore(' ').substringAfterLast('/')
            val countWord = if (rule.count == 1) "one" else rule.count.toString()
            problems += "expected exactly $countWord $name call" + if (rule.count == 1) "" else "s"
        }
    }
